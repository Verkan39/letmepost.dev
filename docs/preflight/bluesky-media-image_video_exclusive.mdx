...
Send two posts. The second can quote-post or thread-reply to the first if you want them visually linked. If you need mixed media in a single visual, [Threads](/platforms/threads) is the platform that supports that — same `media` shape, different `account.platform`.
