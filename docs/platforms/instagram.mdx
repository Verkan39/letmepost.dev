...
<Tip>
- **JPEG only** for images. PNG, WebP, HEIC, and GIF will be rejected with `code 100, error_subcode 2207003`. Preflight catches this locally so you don't lose a roundtrip — re-encode to JPEG via your image pipeline (or upload via [`POST /v1/media`](/api-reference) which preserves the source mime).
...
- [`POST /v1/accounts/connect/instagram`](/api-reference) — start an Instagram OAuth flow.
